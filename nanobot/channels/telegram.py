...
class TelegramChannel(BaseChannel):
    ...
    def __init__(self, config: Any, bus: MessageBus):
        ...
        self._topic_titles: dict[tuple[str, int], str] = {}
    ...
    async def _on_topic_event(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if getattr(message, "forum_topic_created", None) or getattr(
    ...
    def _remember_topic_title(self, message) -> None:
        message_thread_id = getattr(message, "message_thread_id", None)
        if message_thread_id is None:
            return

        title: str | None = None
        if created := getattr(message, "forum_topic_created", None):
            title = getattr(created, "name", None)
        elif edited := getattr(message, "forum_topic_edited", None):
            title = getattr(edited, "name", None)

        if title:
            self._topic_titles[(str(message.chat_id), int(message_thread_id))] = title
    ...
    def _build_message_metadata(self, message, user) -> dict:
        ...
        topic_title = None
        if message_thread_id is not None:
            topic_title = self._topic_titles.get((str(message.chat_id), int(message_thread_id)))
        return {
